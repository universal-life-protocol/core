...
# === STEP 3: Parse .procedure (validates multiset) ===
CLAUSES="$TMPDIR/clauses.tsv"
awk -f "$SCRIPT_DIR/proc.awk" "$WORLD/.procedure" > "$CLAUSES"
...
# === STEP 5: Execute interrupts (execution happens ONCE) ===
while IFS=' ' read -r TAG PROC OPEN CLOSE INTR; do
    ...
    # Compute identifiers
    QID="$(printf '%s' "Q|OPEN=$OPEN|INT=$INTR|CLOSE=$CLOSE" | "$SCRIPT_DIR/hash.sh")"
