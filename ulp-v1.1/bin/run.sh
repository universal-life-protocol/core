...
need() {
    [ -f "$ROOT/$1" ] || { echo "error: missing $ROOT/$1" >&2; exit 2; }
}
...
command -v sort >/dev/null 2>&1 || { echo "error: need sort" >&2; exit 127; }
command -v base64 >/dev/null 2>&1 || { echo "error: need base64" >&2; exit 127; }
[ -x bin/hash.sh ] || { echo "error: missing bin/hash.sh" >&2; exit 127; }

# Create output directory
mkdir -p "$OUTDIR"

# Acquire lock for append-only guarantee
LOCKDIR="$OUTDIR/.lock"
if ! mkdir "$LOCKDIR" 2>/dev/null; then
    echo "error: another run in progress (lock: $LOCKDIR)" >&2
    exit 9
fi
...
[ -n "$ENTRY_PROC" ] || {
    echo "error: no entry procedure (pass arg3 or set .interrupt: on_start <proc>)" >&2
    exit 3
}

# Check if interrupt name is allowed
allowed_name() {
    name="$1"
    ...
    if [ -f "$ROOT/.ignore" ]; then
        awk -v n="$name" '$1==n {bad=1} END {exit bad?1:0}' "$ROOT/.ignore" || return 1
    fi
    return 0
}

# Validate identifier-only dotfiles
for f in .genesis .env .atom .manifest .schema .sequence .include .ignore .interrupt .view .record; do
    if [ -f "$ROOT/$f" ]; then
        awk -f bin/canon.awk "$ROOT/$f" >/dev/null || {
            echo "error: $ROOT/$f is not identifier-only" >&2
            exit 2
        }
    fi
done

# Compute World ID (WID) = hash(canonicalized world files)
world_stream() {
    for f in .genesis .env .atom .manifest .schema .sequence .include .ignore .interrupt .view .record; do
        if [ -f "$ROOT/$f" ]; then
            printf 'FILE %s\n' "$f"
...
# Parse .procedure clauses (validates multiset)
CLAUSES="$TMPDIR/clauses.tsv"
awk -f bin/proc.awk "$ROOT/.procedure" > "$CLAUSES" || {
    echo "error: procedure parsing failed" >&2
    exit 7
}
...
while IFS='	' read -r TAG PROC OPEN CLOSE INTR; do
    ...
    allowed_name "$INTR" || {
        echo "error: interrupt $INTR blocked by include/ignore" >&2
        exit 8
    }

    HANDLER="interrupts/$INTR.sh"
    [ -x "$HANDLER" ] || {
        echo "error: missing interrupt handler: $HANDLER" >&2
        exit 10
    }

    # Compute clause and execution IDs
    QID="$(printf '%s' "Q|OPEN=$OPEN|INT=$INTR|CLOSE=$CLOSE" | bin/hash.sh)"
    EID="$(printf '%s' "E|W=$WID|Q=$QID|INT=$INTR" | bin/hash.sh)"

    # Record clause and execution
    {
