// sw.js - Service Worker for ULP P2P Server PWA
const CACHE_NAME = 'p2p-server-v2.0.0';
const urlsToCache = [
  '/',
  '/manifest.json',
  '/icon-192.png',
...
function openDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('p2p-server', 1);
...
self.addEventListener('push', event => {
  ...
  const title = data.title || 'ULP P2P Server';
  const options = {
