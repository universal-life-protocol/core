...
cd server
go mod tidy
go build -o p2p-server

# Serve ULP traces from ../../out
./p2p-server -traces=../../out -port=8080
```

Output:
```
ULP P2P Server Server started
...
### 2. Start ULP P2P Server

```bash
./p2p-server -traces=../../out
```
...
What it does:
- Starts `p2p-server` against `../../out`
- Waits for the HTTP API to come up
- Lists records and grabs the first RID
- Downloads that trace and verifies SHA-256 matches the RID

### Production (with TLS)

```bash
# Behind nginx/caddy with TLS
./p2p-server -traces=/var/lib/ulp/traces -port=9000
```
...
CMD ["./p2p-server", "-traces=/traces", "-port=8080"]
```

```bash
docker build -t p2p-server .
docker run -p 8080:8080 -p 9090:9090/udp -v ./traces:/traces p2p-server
```
...
### Netlink permission error on startup

**Problem**: `failed to resolve local interface addresses: route ip+net: netlinkrib: operation not permitted`

**Solutions**:
1. Run on a host with network stack access (not a heavily sandboxed container)
2. If containerized, add `--cap-add=NET_ADMIN --network=host` or equivalent to allow interface discovery
...
**ULP P2P Server: Trustless ULP browsing from any device.**
