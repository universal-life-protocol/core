...
# blackboard-web must be a Consumer/Projector only: no port-matroid authority append/reconcile/ingest.
...
if [ -n "$hits" ]; then
  echo "blackboard-web authority gate failed: found port-matroid/append/reconcile/ingest usage:" >&2
  echo "$hits" >&2
  exit 1
fi

echo "ok blackboard-web no-authority gate"
