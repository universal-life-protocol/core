#!/usr/bin/awk -f
#
# poly.awk — ULP v3.0 Execution Algebra
...
function sort_mono_list(arr, n,    i, j, tmp) {
    for (i = 1; i <= n; i++) {
        for (j = i + 1; j <= n; j++) {
            if (cmp_mono(arr[i], arr[j]) > 0) {
                tmp = arr[i]
                arr[i] = arr[j]
                arr[j] = tmp
            }
...
function sorted_mono_keys(map, out_arr,    k, n) {
    n = 0
    for (k in map) {
        out_arr[++n] = k
    }
    sort_mono_list(out_arr, n)
    return n
}
...
function emit_decompile(    n, i, mcount, mono_list, j) {
    ...
    for (i = 1; i <= n; i++) {
        ...
        sort_mono_list(mono_list, mcount)
        for (j = 1; j <= mcount; j++) {
            m = mono_list[j]
